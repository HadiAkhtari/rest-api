...
type Event struct {
	Id          int    `json:"id"`
	OwnerId     int    `json:"ownerid" binding:"required" db:"owner_id"`
	...
	Date        string `json:"date" binding:"required,datetime=2006-01-02" db:"date"`
	Location    string `json:"location" binding:"required,min=3" db:"location"`
}

func (m EventsModel) Insert(event *Event) error {
...
func (m EventsModel) Get(id int) (*Event, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (m EventsModel) Update(event *Event) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (m EventsModel) Delete(id int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (m EventsModel) GetByAttendee(attendeeId int) ([]Event, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	query := `SELECT e.id, e.owner_id, e.name, e.description, e.date, e.location
FEROM events e
JOIN attendees a ON e.id = a.owner_id
WHERE a.user_id = $1`
