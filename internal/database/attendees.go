...
func (m *AttendeeModel) Insert(attendee *Attendee) (*Attendee, error) {
	...
	err := m.DB.QueryRowContext(ctx, query, attendee.Event_Id, attendee.User_Id).Scan(&attendee.Id)
	if err != nil {
		return nil, err
	}
	return attendee, nil
...
func (m *AttendeeModel) GetByEventAndAttendee(eventId, userId int) (*Attendee, error) {
	...
	err := m.DB.QueryRowContext(ctx, query, eventId, userId).Scan(&attendee.Id, &attendee.User_Id, &attendee.Event_Id)
	if err != nil {
		return nil, err
	}
	return &attendee, nil
...
func (m *AttendeeModel) Delete(userId, eventId int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	query := `DELETE FROM users WHERE user_id=$1 AND event_id=$2`
